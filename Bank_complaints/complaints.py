...
def complaints_per_capita_year(state) : 
        cpc_state = (complaints_per_capita[(complaints_per_capita["State"] == state)]
                                        [["State", "Year", "Complaints_per_100k"]])
        cpc_state = cpc_state.sort_values("Year").reset_index(drop = True)
        cpc_state.index = cpc_state.index +1 
        return cpc_state
"""
california = complaints_per_capita[complaints_per_capita["State"] == "California"]

plt.bar(
      california["Year"], 
      california["Complaints_per_100k"]
)
"""
# returns complaint per capita for the selected state
print((complaints_per_capita_year("District of Columbia")["Complaints_per_100k"].pct_change() * 100).round(2).astype(str) + "%")

def complaints_state_issue(state,year): 
    df = clean_complaints[
        (clean_complaints["State"] == state) &
        (clean_complaints["Date_received"].dt.year == year)][["Issue"]]
    
    return df
"""
plt.bar(complaints_per_capita_year("District of Columbia")["Year"], complaints_per_capita_year("District of Columbia")["Complaints_per_100k"])
plt.show()
"""

clean_complaints.to_csv(desktop/ "clean_complaints.csv", index = False)
#population.to_csv(desktop/ "clean_population.csv", index = False)
